...
double vol(const Eigen::Vector3d &a, const Eigen::Vector3d &b, const Eigen::Vector3d &c, const Eigen::Vector3d &d) {
    Eigen::Matrix3d A;
    A.col(0) = b - a;
    A.col(1) = c - a;
    A.col(2) = d - a;
...
Eigen::Vector3d cross(const Eigen::Vector3d &v0,
                      Eigen::Vector3d &v1) {
...
tetrahedralize(const std::vector<Eigen::Vector3d> &points,
    ...
    for (const auto &face: faces) {
        ...
        for (int j = 0; j < (int) face.size(); j++) {
            std::vector<int> tet;
            if (vol( outPts[face[j]], outPts[face[(j + 1) % (np_face)]], outPts[np + i],outPts[tet_np - 1]) > 0) {
                ...
                tet.emplace_back(tet_np - 1);
                outTets.emplace_back(tet);
                if (vol(outPts[tet_np - 1], outPts[face[(j + 1) % (np_face)]], outPts[np + i], outPts[face[j]]) < 0) {

...
//consider explicit triangle mesh (for butterfly subdividison)
void setup_diamond_gradient_divergence_intrinsic_mullification(SurfaceMesh &mesh, Eigen::SparseMatrix<double> &G,Eigen::SparseMatrix<double> &D, double delta)
{
    Eigen::SparseMatrix<double> M;
    std::vector<Triplet> triplets, triplets_area;
    const int nv = mesh.n_vertices();
    ...
    double moll_eps = mollification_epsilon(mesh,delta);
    Vertex v, vv;
    Face f, ff;
    ...
    for (Edge e : mesh.edges())
    {
        if (!mesh.is_boundary(e))
        {
            ...
            v = mesh.from_vertex(h0);
            vv = mesh.to_vertex(h0);

            F1 = mesh.position(v);
            F2 = mesh.position(vv);

            T1 =  mesh.position(mesh.to_vertex(mesh.next_halfedge(h0)));
            T2 =  mesh.position(mesh.to_vertex(mesh.next_halfedge(h1)));
        }
        else
        {
            Halfedge he = mesh.halfedge(e, 0);
            ...
            if (!mesh.is_boundary(he))
            {
                T2 = mesh.position(mesh.to_vertex(mesh.next_halfedge(he)));
                T1 = 0.5 * (F1 + F2);
            }
            else
            {
                T2 = mesh.position(mesh.to_vertex(mesh.next_halfedge(mesh.opposite_halfedge(he))));
                T1 = 0.5 * (F1 + F2);
            }
        }
        ...
        f1 = vec2(0.0, 0.0);
        f2 = vec2(0.0, norm(F1 - F2)+moll_eps);

        t2 = dot((T2 - F1), e1) * vec2(1.0, 0) +
             dot((T2 - F1), e2) * vec2(0.0, 1.0);


        t2 *=  (norm(t2)+moll_eps)/norm(t2) ;
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            if (!mesh.is_boundary(e))
            {
                ...
                triplets.emplace_back(row, mesh.to_vertex(mesh.next_halfedge(h0)).idx(), G2(i, 3));
                triplets.emplace_back(row, mesh.to_vertex(mesh.next_halfedge(h1)).idx(), G2(i, 1));
            }
            else
            {
                Halfedge he = mesh.halfedge(e, 0);
                if (!mesh.is_boundary(he))
                {
                    triplets.emplace_back(row, mesh.to_vertex(mesh.next_halfedge(he)).idx(), G2(i, 1));
                    triplets.emplace_back(row, vv.idx(), 0.5 * G2(i, 3));
                    triplets.emplace_back(row, v.idx(), 0.5 * G2(i, 3));
                }
                else
                {
                    triplets.emplace_back(row, mesh.to_vertex(mesh.opposite_halfedge(he)).idx(), G2(i, 1));
                    triplets.emplace_back(row, vv.idx(), 0.5 * G2(i, 3));
                    triplets.emplace_back(row, v.idx(), 0.5 * G2(i, 3));
                }
            }
...
void setup_diamond_mass_matrix_intrinsic_mullification(SurfaceMesh &mesh,Eigen::SparseMatrix<double> &M, double delta){
        ...
        for (Edge e : mesh.edges())
        {
            if (!mesh.is_boundary(e))
            {
                ...
                f1_idx = v.idx();
                f2_idx = vv.idx();
                t1_idx = mesh.to_vertex(mesh.next_halfedge(h0)).idx();
                t2_idx = mesh.to_vertex(mesh.next_halfedge(h1)).idx();
            ...
            else
            {
                ...
                if (!mesh.is_boundary(he))
                {
                    T2 = mesh.position(mesh.to_vertex(mesh.next_halfedge(he)));
                    T1 = 0.5 * (F1 + F2);
                    t2_idx = mesh.to_vertex(mesh.next_halfedge(he)).idx();
                }
                else
                {
                    T2 = mesh.position(mesh.to_vertex(mesh.next_halfedge(mesh.opposite_halfedge(he))));
                    T1 = 0.5 * (F1 + F2);
                    t2_idx = mesh.to_vertex(mesh.next_halfedge(mesh.opposite_halfedge(he))).idx();
...
double mollification_epsilon( SurfaceMesh &mesh, double delta){
    ...
    for(auto f : mesh.faces()){
        ...
        double eps_loc = delta + mesh.edge_length(mesh.edge(h0))- mesh.edge_length(mesh.edge(h1))- mesh.edge_length(mesh.edge(h2));
        if(eps_loc > eps){
            eps = eps_loc;
        }
    }

...
bool VolumeMeshIO::write_histrogram()
{
    ...
    if (ext == "hybrid")
    {
            ...
            if (c > nv + nf && c <= nv + nf + np)
            {
                ...
                if ((c - nv + nf) % 2 == 0)
                {
                    ...
                    else
                    {
                        ...
                        hist.insert(lb,
                                    MapType::value_type(
                                        k, v)); // Use lb as a hint to insert,
        ...
        std::string hist_name =
            filename_.substr(slash + 1, dot - 1) + "_histogram.txt";
        ...
        for (const auto& it : tuples)
        {
            ...
            if (f == face_map.end())
            {
                ...
                if (((midP - _mesh.vertex(it[0])) | n) > 0.0)
                {
                    ...
                    cell_halffaces.push_back(
                        VolumeMesh::halfface_handle(fh, 0));
                }
                else
                {
                    ...
                    // Add corresponding half-face to cell definition
                    cell_halffaces.push_back(
                        VolumeMesh::halfface_handle(fh, 1));
                }
            }
            else
            {

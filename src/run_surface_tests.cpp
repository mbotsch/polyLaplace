...
void write_data(SurfaceMesh& mesh, int laplace, std::ofstream& file,
                int function, bool spheredist, int l = 2, int m = 0)
{
    if (function == curvature)
    {
        ...
        file << analyzer.compute_curvature_error(laplace, AreaMinimizer,
                                                 lumped);
        file << ",";
    }
    else if (function == SH)
    ...
    {
        double error =
            solve_poisson_system(mesh, laplace, AreaMinimizer, function, l, m);
        file << error;
        file << ",";
    }
    else if (function == Geodesics)
    {
        ...
        {
            GeodesicsInHeat heat(mesh, laplace, AreaMinimizer, spheredist,
                                 !spheredist, MaxDiagonal);
...
{
    file_error << "[AW11] l=2,[AW11] l=1,[AW11] l=0.5,[AW11] l=0.1,[dGBD20] "
                  "l=2,[dGBD20] l=1,[dGBD20] l=0.5,[dGBD20] "
                  "l=0.1,[BHKB20],[BBA21],[MKB08],MEL"
               << std::endl;
}
...
void write_kernelProbe_convergence_test_csv(int lvl = 6)
{
    ...
    SurfaceMesh mesh;
    file_franke_voronoi << "nk=2 np=6,nk=2 np=8,nk=2 np=10,nk=5 np=15,nk=5 "
                           "np=20,nk=5 np=25,nk=10 np=30,nk=10 np=40,nk=10 "
                           "np=50,nk=20 np=60,nk=20 np=80,nk=20 np=100,MEL"
                        << std::endl;
    int voronoi_lvl = lvl;
    if (lvl > 6)
        voronoi_lvl = 6;
    ...
    {
        std::string meshname = "../data/surface_meshes/grid/clean/voronoi_" +
                               std::to_string(j) + ".obj";
        read(mesh, meshname);
        ...
        {
            b(v.idx()) =
                laplace_of_poisson_function(mesh.position(v), Franke2d);
        }
        int n = 4;
        int nKernel, nProbes;
            ...
            for (int np = 0; np < 3; np++)
            {
                ...
                for (auto v : mesh.vertices())
                {
                    if (mesh.is_boundary(v))
                    {
                        ...
                        B(v.idx()) =
                            poisson_function(mesh.position(v), Franke2d);
                    }
                }
                ...
                for (unsigned int i = 0; i < S.outerSize(); i++)
                    for (Eigen::SparseMatrix<double>::InnerIterator iter(S, i);
                         iter; ++iter)
                    {
                        Vertex row = Vertex(iter.row());
                        Vertex col = Vertex(iter.col());
                ...
                for (auto v : mesh.vertices())
                {
                    error += pow(x[v.idx()] - poisson_function(mesh.position(v),
                                                               Franke2d),
                                 2.);
                }

                std::cout << "Franke RMSE error inner vertices: "
                          << sqrt(error / (double)mesh.n_vertices())
                          << std::endl;
    ...
    file_concave << "nk=2 np=6,nk=2 np=8,nk=2 np=10,nk=5 np=15,nk=5 np=20,nk=5 "
                    "np=25,nk=10 np=30,nk=10 np=40,nk=10 np=50,nk=20 "
                    "np=60,nk=20 np=80,nk=20 np=100,MEL"
                 << std::endl;
    for (int j = 1; j < lvl; j++)
    {
        std::string meshname =
            "../data/surface_meshes/sphere/hex_" + std::to_string(j) + ".obj";
        read(mesh, meshname);
        std::cout << meshname << std::endl;
        ...
        for (int nk = 0; nk < n; nk++)
        {
            for (int np = 0; np < 3; np++)
            {
                ...
                double eval = -l * (l + 1);
                error =
                    (b - 1.0 / eval * X).transpose() * M * (b - 1.0 / eval * X);
...
void write_convergence_data_csv(Function function, int lvl = 7,
                                int start_lvl = 1, int l = 2, int m = 0)
{
    SurfaceMesh mesh;
    ...
    if (function != Franke2d)
    {
        ...
        for (int i = start_lvl; i < lvl; i++)
        {
            ...
            normalize(mesh);
            double res = inverse_mean_edgelenth(mesh);
            write_all_laplace_data(mesh, file_concave, function, sphere_dist, l,
                                   m);
        ...
        for (int i = start_lvl; i < lvl; i++)
        {
            ...
            double res = inverse_mean_edgelenth(mesh);
            write_all_laplace_data(mesh, file_noise, function, sphere_dist, l,
                                   m);
        ...
        file_noise_quad
            << "[AW11] l=2,[AW11] l=1,[AW11] l=0.5,[AW11] l=0.1,[dGBD20] "
               "l=2,[dGBD20] l=1,[dGBD20] l=0.5,[dGBD20] "
               "l=0.1,[BHKB20],[BBA21],[MKB08],Mean plane distance"
            << std::endl;
        for (int i = start_lvl; i < 9; i++)
        {
            ...
            write_all_laplace_data(mesh, file_noise_quad, function, sphere_dist,
                                   l, m);
                                ...
                                const std::string& filename)
{
    double rmse;
    poly_laplace_lambda_ = 2.0;
    rmse = solve_eigenvalue_problem(mesh, AlexaWardetzkyLaplace, AreaMinimizer,
                                    filename);
    file << rmse << ",";
    poly_laplace_lambda_ = 1.0;
    rmse = solve_eigenvalue_problem(mesh, AlexaWardetzkyLaplace, AreaMinimizer,
    ...
    deGoes_laplace_lambda_ = 2.0;
    rmse =
        solve_eigenvalue_problem(mesh, deGoesLaplace, AreaMinimizer, filename);
    file << rmse << ",";
    deGoes_laplace_lambda_ = 1.0;
    rmse =
        solve_eigenvalue_problem(mesh, deGoesLaplace, AreaMinimizer, filename);
    ...
    rmse =
        solve_eigenvalue_problem(mesh, deGoesLaplace, AreaMinimizer, filename);
    file << rmse << ",";
    rmse = solve_eigenvalue_problem(mesh, PolySimpleLaplace, AreaMinimizer,
                                    filename);
    file << rmse << ",";
...
void evaluate_conditionNr_per_mesh(SurfaceMesh& mesh, std::ofstream& file)
{
    ...
    poly_laplace_lambda_ = 2.0;
    result =
        condition_number(mesh, AlexaWardetzkyLaplace, AreaMinimizer, condnr);
    file << result << ",";
    poly_laplace_lambda_ = 1.0;
    result =
        condition_number(mesh, AlexaWardetzkyLaplace, AreaMinimizer, condnr);
...
void write_evalue_test_results(int lvl = 7)
{
    ...
    error_file << "[AW11] l=2,[AW11] l=1,[AW11] l=0.5,[AW11] l=0.1,[dGBD20] "
                  "l=2,[dGBD20] l=1,[dGBD20] l=0.5,[dGBD20] "
                  "l=0.1,[BHKB20],[BBA21],[MKB08]\n";
    for (int i = 2; i < lvl; i++)
    {
        SurfaceMesh mesh;
        std::string meshname;
        meshname =
            "../data/surface_meshes/sphere/quad_" + std::to_string(i) + ".obj";
    ...
    error_file_hex << "[AW11] l=2,[AW11] l=1,[AW11] l=0.5,[AW11] "
                      "l=0.1,[dGBD20] l=2,[dGBD20] l=1,[dGBD20] l=0.5,[dGBD20] "
                      "l=0.1,[BHKB20],[BBA21],[MKB08]\n";
    for (int i = 2; i < lvl; i++)
    {
        SurfaceMesh mesh;
        std::string meshname;
        meshname =
            "../data/surface_meshes/sphere/hex_" + std::to_string(i) + ".obj";
    ...
    error_file_triangle << "[AW11] l=2,[AW11] l=1,[AW11] l=0.5,[AW11] "
                           "l=0.1,[dGBD20] l=2,[dGBD20] l=1,[dGBD20] "
                           "l=0.5,[dGBD20] l=0.1,[BHKB20],[BBA21],[MKB08]\n";
    for (int i = 2; i < lvl; i++)
    {
        SurfaceMesh mesh;
        std::string meshname;
    ...
    error_file_concave << "[AW11] l=2,[AW11] l=1,[AW11] l=0.5,[AW11] "
                          "l=0.1,[dGBD20] l=2,[dGBD20] l=1,[dGBD20] "
                          "l=0.5,[dGBD20] l=0.1,[BHKB20],[BBA21],[MKB08]\n";
    for (int i = 2; i < lvl; i++)
    {
...
void write_conditionNR_test_results()
{
    ...
    error_file << "[AW11] l=2,[AW11] l=1,[AW11] l=0.5,[AW11] l=0.1,[dGBD20] "
                  "l=2,[dGBD20] l=1,[dGBD20] l=0.5,[dGBD20] "
                  "l=0.1,[BHKB20],[BBA21],[MKB08],Inverse Edge lenght\n";
    SurfaceMesh mesh;
    std::string meshname;
    meshname = "../data/surface_meshes/grid/quad_" + std::to_string(i) + ".obj";
    read(mesh, meshname);
    Edge origin_edge;
    auto min = DBL_MAX;
    for (auto e : mesh.edges())
    ...
    meshname =
        "../data/surface_meshes/grid/voronoi_" + std::to_string(i) + ".obj";
    read(mesh, meshname);
    min = DBL_MAX;
    for (auto e : mesh.edges())
    {
    ...
    meshname =
        "../data/surface_meshes/grid/triangle_" + std::to_string(i) + ".obj";
    filename = "triangle_" + std::to_string(i);
    read(mesh, meshname);
    min = DBL_MAX;
    for (auto e : mesh.edges())
    ...
    meshname =
        "../data/surface_meshes/grid/concave_" + std::to_string(i) + ".obj";
    read(mesh, meshname);
    min = DBL_MAX;
    for (auto e : mesh.edges())
    {

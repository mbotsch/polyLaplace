#include "HarmonicPolyhedron.h"
#include "Tetrahedralize.h"
#include <iostream>
#include <utility>

void loadPoly(const std::string& fname, std::vector<Eigen::Vector3d>& vertices,
...
HEdge::HEdge(const int i_, const int j_)
{
    if (i_ < j_)
    {
        i = i_;
...
bool HEdge::operator==(const HEdge& e2) const
{
...
Normal::Normal() : n(Eigen::Vector3d::Zero()) {}
...
void HarmonicPolyhedron::dumpProbesAndKernels()
{
    ...
    for (const auto& v : kernelCenters)
        fk << ((v + mean) / scale).transpose() << std::endl;
    fk.close();
    ...
    for (int i = 0; i < (int)vertices.size(); ++i)
        vertexMatrix.row(i) = vertices[i].transpose();
    Eigen::MatrixXd evSamples = PEdgeSamples * vertexMatrix;

    for (int i = 0; i < evSamples.rows(); ++i)
    {
        fs << Eigen::RowVector3d((evSamples.row(i).transpose() + mean) / scale)
           << std::endl;
    }
...
void HarmonicPolyhedron::dumpCrossection()
{
    ...
    for (int i = 0; i < N; ++i)
    {
        for (int j = 0; j < N; ++j)
        {
            Eigen::Vector3d s3d(-1. + 2. * i / (double)N, .0,
                                -1. + 2. * j / (double)N);
...
bool HarmonicPolyhedron::insideConvex(const Eigen::Vector3d& p)
{
    ...
    {
        if (p.dot(harmonicFace.getNormal()) > harmonicFace.getD())
            return false;
    }
...
Eigen::Vector3d HarmonicPolyhedron::evaluateGrad(const int node,
                                                 const Eigen::Vector3d& p)
{
    if (node < 0 || node >= coefficients.rows())
...
void HarmonicPolyhedron::initQuadrature()
{
    ...
    tetrahedralize(vertices, faces, tets);
...
void HarmonicPolyhedron::stiffnessMatrix(Eigen::MatrixXd& K)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j <= i; ++j)
        {
            double val = std::pow(1. / scale, 1) *
                         tetQuad.apply([&](const Eigen::Vector3d& p) {
                             return evaluateGrad(i, p).dot(evaluateGrad(j, p));
                         });
...
void HarmonicPolyhedron::massMatrix(Eigen::MatrixXd& M)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j <= i; ++j)
        {
            double val = std::pow(1. / scale, 3) *
                         tetQuad.apply([&](const Eigen::Vector3d& p) {
                             return evaluate(i, p) * evaluate(j, p);
                         });
...
HarmonicPolyhedron::HarmonicPolyhedron() = default;
...
HarmonicPolyhedron::HarmonicPolyhedron(std::vector<Eigen::Vector3d> vertices_,
                                       std::vector<std::vector<int>> faces_)
    : vertices(std::move(vertices_)), faces(std::move(faces_))
{
    init();
}
...
void HarmonicPolyhedron::init()
{
    ...
    for (auto& v : vertices)
        scale = std::max(scale, (v - mean).norm());
    scale = 1. / scale;
    ...
    for (auto& f : faces)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            triPoints.push_back(vector<Eigen::Vector3d>{
                vertices[f[i]], vertices[f[(i + 1) % n]], mean_});
        }
    }
    ...
    for (auto& hf : harmonicFaces)
    {
        ...
        for (auto& s : samples)
            kernelCenters.push_back(s + eps * hf.getNormal());
    }
    ...
    for (auto& e : edges)
    {
        ...
        for (int i = 1; i < kernelsPerEdge; ++i)
        {
            ...
            kernelCenters.push_back((1. - w) * vertices[e.i] +
                                    w * vertices[e.j] + eps * nrml);
        }
    }
    ...
    for (int i = 0; i < (int)vertices.size(); ++i)
    {
        ...
        {
            const auto vals = harmonicFaces[j].evaluateAtPoints(
                globalToLocalIdMap(i, j), faceSamples2d[j]);
            for (auto x : vals)
                b(cnt++) = x;
        }

        // now evaluate at edge/vertex probes
        b.bottomRows(PEdgeSamples.rows()) =
            PEdgeSamples * Eigen::VectorXd::Unit(vertices.size(), i);

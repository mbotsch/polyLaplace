...
class HarmonicPolygon
{
    ...
    Eigen::MatrixXd
        coefficients; // for each of the 'n' functions: nKernels + 3 coefficients. One for each kernel and three for the linear part.
...
public:
    ...
    std::vector<Eigen::Vector3d> unproject(
    ...
    std::vector<double> evaluateAtPoints(
        int id, const std::vector<Eigen::Vector2d>& points);

    void dump2d(const std::string& fname);

    explicit HarmonicPolygon(const Eigen::MatrixXd& pts_);

    explicit HarmonicPolygon(const Eigen::MatrixXd& pts_, int nKernel,
                             int nProbes);

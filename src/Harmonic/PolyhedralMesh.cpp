...
bool PolyhedralMesh::loadOVM(const std::string& filename)
{
    ...
    auto CheckString = [&](const std::string& s) {
        ...
        if (s.compare(buf))
        {
            std::cout << "reading ovm: string " << s << " not found. Returning."
                      << std::endl;
            return false;
        }
    ...
    if (!CheckString("OVM") || !CheckString("ASCII") ||
        !CheckString("Vertices"))
        return false;
...
std::vector<PolyhedralMesh::Index> PolyhedralMesh::getCellGeometry(
    const Index cellIndex, std::vector<Eigen::Vector3d>& cellPoints,
    std::vector<std::vector<Index>>& cellFaces, bool triangularize) const
{
    std::vector<Point> pts;
    auto ret = getCellGeometry(cellIndex, pts, cellFaces, triangularize);

    for (auto& p : pts)
        cellPoints.emplace_back(p[0], p[1], p[2]);

    return ret;
}

std::vector<PolyhedralMesh::Index> PolyhedralMesh::getCellGeometry(
    const Index cellIndex, std::vector<Point>& cellPoints,
    std::vector<std::vector<Index>>& cellFaces, bool triangularize) const
{
    auto cfaces = cellfaceIndizes(cellIndex);

    vector<Index> vertices;
    for (auto& f : cfaces)
        copy(f.begin(), f.end(), back_inserter(vertices));
    ...
    for (auto& f : cfaces)
    {
        ...
        for (int k : f)
            polyVerts.push_back(vertexMap[k]);
        ...
        if (triangularize)
        {
            ...
            {
                cellFaces.push_back(
                    {polyVerts[0], polyVerts[i], polyVerts[i + 1]});
            }
        }
        else
            cellFaces.push_back(polyVerts);
    }

    return vertices;
}

vector<PolyhedralMesh::Index> PolyhedralMesh::faceIndizes(
    const HalfIndex& halfFace) const
{
...
vector<vector<PolyhedralMesh::Index>> PolyhedralMesh::cellfaceIndizes(

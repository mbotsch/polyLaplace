...
double
volume_tetrahedron_signed(VolumeMesh::PointT i, VolumeMesh::PointT j, VolumeMesh::PointT k, VolumeMesh::PointT l) {
...
void compute_3D_virtual_points_and_weights(VolumeMesh &mesh, int face_point, int cell_point) {
...
//        std::vector<Eigen::Vector3d> cell_v; // Container for cell vertices
        ...
        for (auto c_f_it = c_f_it_pair.first; c_f_it != c_f_it_pair.second; ++c_f_it) {
            ...
            cell_v.emplace_back(min); // Virtual face vertex is part of the affine combination for the volume minimizer
    ...
    for (const auto & tetrahedron : tetrahedrons) {
        ...
        for (int k = 0; k < 3; k++) {
            for (int l = 0; l < 3; l++) {
                ...
                if (k == l) {
                    J(k, k) += 2.0 * d23 * d23 - d23 * (d24 * 4.0 + d34 * 4.0 + 8.0 * (c(k) - d(k)) * (d(k) - b(k)))
                               + 2.0 * d24 * d24 - d24 * (d34 * 4.0 + 8.0 * (d(k) - c(k)) * (c(k) - b(k)))
                               + 2.0 * d34 * d34 + d34 * 8.0 * (d(k) - b(k)) * (c(k) - b(k));
                } else {

                    J(k, l) += -d23 * (4.0 * (d(k) - b(k)) * (c(l) - d(l)) + 4.0 * (c(k) - d(k)) * (d(l) - b(l)))
                               - d24 * (4.0 * (c(k) - b(k)) * (d(l) - c(l)) + 4.0 * (d(k) - c(k)) * (c(l) - b(l)))
                               + d34 * (4.0 * (c(k) - b(k)) * (d(l) - b(l)) + 4.0 * (d(k) - b(k)) * (c(l) - b(l)));
                }
            }
            b_(k) += -2.0 * d23 * d23 * d(k) - d23 * (d24 * (-2.0 * (c(k) + d(k))) + d34 * -2.0 * (b(k) + d(k))
                                                      + 2.0 * (d(k) - b(k)) * (d.squaredNorm() - c.squaredNorm()) +
                                                      2.0 * (c(k) - d(k)) * (b.squaredNorm() - d.squaredNorm()))
                     - 2.0 * d24 * d24 * c(k) - d24 * (-d34 * 2.0 * (b(k) + c(k))
                                                       + 2.0 * (c(k) - b(k)) * (c.squaredNorm() - d.squaredNorm()) +
                                                       2.0 * (d(k) - c(k)) * (b.squaredNorm() - c.squaredNorm()))
                     - 2.0 * d34 * d34 * b(k) + d34 * (
...
void find_weights_for_point_3d(const std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> > &poly,
...
VolumeMesh::PointT compute_triangle_normal(VolumeMesh::PointT a, VolumeMesh::PointT b, VolumeMesh::PointT c) {

...
void HarmonicPolyhedron::stiffnessMatrix(Eigen::MatrixXd &K) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j <= i; ++j) {
            double val = std::pow(1. / scale, 1) * tetQuad.apply(
                    [&](const Eigen::Vector3d &p) { return evaluateGrad(i, p).dot(evaluateGrad(j, p)); });
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j <= i; ++j) {
            double val = std::pow(1. / scale,  3) *
                         tetQuad.apply([&](const Eigen::Vector3d& p) { return evaluate(i, p) * evaluate(j, p); });
...
void HarmonicPolyhedron::init() {
    ...
    for (auto &f: faces) {
        ...
        for (int i = 0; i < n; ++i) {
            triPoints.push_back(vector<Eigen::Vector3d>{vertices[f[i]], vertices[f[(i + 1) % n]], mean_});
        }
    }

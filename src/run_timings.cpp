...
double StiffnessConstructionTimings2D(SurfaceMesh &mesh, int laplace){
    ...
    t.stop();
    std::cout << "Time for Laplace Construction : " << t.elapsed()/20.0 << "ms" << std::endl;
...
double StiffnessConstructionTimings3D(VolumeMesh &mesh, int laplace){
    ...
    for (int i = 0; i < 20; i++){
        setup_3D_stiffness_matrix(mesh,L,laplace,Quadratic_Areas_,Quadratic_Volume_);
    }
    t.stop();
    std::cout << "Time for Laplace Construction : " << t.elapsed()/20.0 << "ms" << std::endl;
    return t.elapsed()/20.0;
...
void takeLaplaceTiming2D( SurfaceMesh &mesh, int laplace, std::ofstream &file_timings){
    ...
    setup_stiffness_matrices(mesh,L,laplace,Quadratic_Areas_);
    t_stiffness.stop();
    std::cout << "Time for Laplace Construction : " << t_stiffness.elapsed() << "ms" << std::endl;
    ...
    std::cout << "Ratio cholesky to system : " << C.nonZeros()/(float)A.nonZeros() << endl;
    std::cout << "Time for compute A : " << t.elapsed()/10.0 << "ms" << std::endl;
    ...
    std::cout << "Time for solving CMCf : " << t_solve.elapsed()/10.0 << "ms" << std::endl;
}


void takeLaplaceTiming3D(const std::string& meshname, int Laplace, std::ofstream &file_timings){
    ...
    if(Laplace == Harmonic3D){
        ...
        std::cout << "Time for Laplace Construction : " << t_stiffness.elapsed() << "ms" << std::endl;
        file_timings << t_stiffness.elapsed() << ",";
        setup_3D_mass_matrix(mesh, M, Laplace, Quadratic_Areas_, Quadratic_Volume_);
    }
    ...
    std::cout << "Nonzeros Cholesky: " << C.nonZeros() << std::endl;
    std::cout << "Ratio cholesky to system : " << C.nonZeros()/(float)A.nonZeros() << endl;
    std::cout << "Time for compute A : " << t_solve.elapsed()/10.0 << "ms" << std::endl;

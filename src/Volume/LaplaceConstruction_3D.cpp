                               ...
                               int Laplace, int face_point, int cell_point)
{
    ...
    {
        auto f_w_prop =
            mesh.request_face_property<Eigen::VectorXd>("face weights");
        auto c_w_prop =
            mesh.request_cell_property<Eigen::VectorXd>("cell weights");
        auto f_prop =
            mesh.request_face_property<VolumeMesh::PointT>("face points");
        auto c_prop =
            mesh.request_cell_property<VolumeMesh::PointT>("cell points");
    ...
    else if (Laplace == PolySimpleLaplace)
    {
        ...
        compute_3D_virtual_points_and_weights(mesh, face_point, cell_point);
        setup_3D_cell_face_prolongation_matrix(mesh, P, Pc, Pf);
        setup_3D_polysimple_stiffness_matrix(mesh, S, Sf, face_point,
                                             cell_point);
    }
}
